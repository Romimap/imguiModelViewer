...
//~~~Not filtered
	...
	//~~~Trilinear filtering
	if (maxAniso == 1) {
		return textureLod(tex, uv, log2(pmax));
	}
	
*/

vec4 gtexture(sampler2D tex, vec2 uv, int maxAniso = 16) {
...
/*	vec3 COL = vec3(0);
	COL[int(lambda)%3]=0.3;
	return vec4(COL*float(int(lambda)/3+1),1);
	*/
	...
	for (int i = 1; i <= NB; i += 1) {
		...
		vec2 curr_uv = vec2(
			u + dudt * (float(i)/N - 0.5), 
			v + dvdt * (float(i)/N - 0.5)
		);
		
		taniso += textureLod(tex, curr_uv, lambda);
	}
...
vec3 lightDirection () {
	return normalize(vec3(1, 0.25, 1)); //0 0.1 1
}
...
vec3 GlobalToNormalSpace(vec3 v) {
	mat3 tangentTransform = mat3(cross(vNormal, vTangent), vNormal, vTangent);
	return tangentTransform * v;
}
...
vec3 NormalToGlobalSpace(vec3 v) {
	mat3 tangentTransform = mat3(cross(vNormal, vTangent), vNormal, vTangent);
	return inverse(tangentTransform) * v;
}
...
float getDiffuseIntensity(float meanx, float meany) {
	...
	return dot(snrm, lightDirection());
}
...
vec3 mixDiffuseSpecular (vec3 diffuse, vec3 specular) {
	return diffuse + specular * 0.05;
}


vec3 groundTruth (int n) {
	float samplesq = n;
	...
	for (int x = 1; x <= samplesq; x++) {
		for (int y = 1; y <= samplesq; y++) {
			vec2 uv = vUv;
			uv += ((x / float(samplesq + 1)) - 0.5) * duvdx * 1;
			uv += ((y / float(samplesq + 1)) - 0.5) * duvdy * 1;

			float t = 0.0;
			t = SpecularTilingBlending(false, false, uv);
			vec3 diffuse = getTilingBlendingDiffuse(vec3(0.4, 0.6, 0.8) * 0.5, 0.3, uv); //0.2 0.3 0.5
		

			vec3 color = mixDiffuseSpecular(diffuse * 0.0, vec3(t));
...
void main () {
	...
	vec3 b = TilingAndBlendingSq(bmap, uv);
	vec3 m = TilingAndBlendingSq(mmap, uv);
	...
	float specularIntensity = getSpecularIntensity (meanx, meany, varx, vary, covxy);
	float diffuseIntensity = getDiffuseIntensity(meanx, meany);


	vec3 color = vec3(0);
	color += vec3(specularIntensity) * 0.015;
	//color += vec3(diffuseIntensity) * 1.0;
	
	color = colorManagement(color, 1);
	//color = gtexture(mipchart, uv).rgb;
	//color = gtexture(albedo, uv).rgb;
